...
p <- new_metrics %>%
  ...
  geom_hline(aes(yintercept = mean, color = .metric), model_metrics,
             alpha = 0.7, size = 1.2, lty = 2) +
  geom_line(aes(color = .metric)) +
  geom_point(aes(color = .metric), size = 2, alpha = 0.8) +
...
https://github.com/tidymodels/modelops-playground
...
```{r}
pal <- colorFactor(c("darkred", "gold"), domain = c("injuries", "none"))
...
This map, like the previous plot, shows the _new_ crashes used for monitoring since the last time this dashboard was generated.
